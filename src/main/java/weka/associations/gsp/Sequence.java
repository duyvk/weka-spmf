...
public class Sequence
  ...
  public static FastVector oneElementsToSequences(FastVector elements) {
    ...
    while (elementEnum.hasMoreElements()) {
      ...
      //seq.setElements(seqElements);
      sequences.addElement(seq);
     // System.out.println(sequences.capacity());
     // printSetOfSequences(sequences);
     // System.out.println("-------------------------------------------------------------------");
    }
    return sequences;
  }
  ...
  public static String setOfSequencesToString(FastVector setOfSequences, Instances dataSet, FastVector filterAttributes) {
    ...
    while(SequencesEnum.hasMoreElements()) { // for each sequence 
      Sequence seq = (Sequence) SequencesEnum.nextElement();  
      Integer filterAttr = (Integer) filterAttributes.elementAt(0);
      printSeq = true; 

      if (filterAttr.intValue() != -1) { //allow to print
		for (int j=0; j < filterAttributes.size(); j++) {// for each filterAttribute element    
		  filterAttr = (Integer) filterAttributes.elementAt(j);
		  FastVector seqElements = seq.getElements(); // get all elements of sequence 
	
		  if (printSeq) {
		    for (int k=0; k < seqElements.size(); k++) {// for each elements of sequence
